...
export const getProperties = async (
  ...
  try {
    ...
    // Fix: Use string[] instead of Prisma.Sql[]
    let whereConditions: string[] = [];

    if (favoriteIds) {
      const favoriteIdsArray = (favoriteIds as string).split(",").map(Number);
      whereConditions.push(
        `p.id IN (${favoriteIdsArray.join(',')})`
      );
    }
    ...
    if (amenities && amenities !== "any") {
      ...
      whereConditions.push(`p.amenities && ARRAY[${amenitiesArray.map(a => `'${a}'`).join(',')}]::"Amenity"[]`);
    }
    ...
    // Fix: Use template literals instead of Prisma.sql
    ...
    const properties = await prisma.$queryRawUnsafe(query);
...
export const createProperty = async (
  ...
  try {
    ...
    // Fix: Define proper type for location result
    type LocationResult = {
      id: number;
      address: string;
      city: string;
